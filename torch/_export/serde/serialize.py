...
from torchgen.model import BaseTy, BaseType
...
def _is_hop_single_tensor_return(node) -> bool:
    ...
    try:
        schema = torch._library.utils.hop_schema_from_fx_node(node)
        returns = schema.returns
        if len(returns) != 1:
            return False
        return_type = returns[0].type
        return isinstance(return_type, BaseType) and (return_type.name is BaseTy.Tensor)
    except Exception:
    ...
    def handle_call_function(self, node: torch.fx.Node):
        ...
        if (
            ...
            ex_node = Node(
                ...
                metadata=self.serialize_metadata(node),
                is_single_tensor_return=_is_hop_single_tensor_return(node),
            )
    ...
    def deserialize_node(self, serialized_node: Node, target: Callable) -> None:
        if (
            ...
            name = (
                ...
                and getattr(serialized_node, "is_single_tensor_return", True)
                else None
            )
    ...
    def deserialize_outputs(self, serialized_node: Node, fx_node: torch.fx.Node):
        ...
        if (
            ...
            if (
                ...
                and not getattr(serialized_node, "is_single_tensor_return", True)
            ):
                meta_val: List[Any] = []
